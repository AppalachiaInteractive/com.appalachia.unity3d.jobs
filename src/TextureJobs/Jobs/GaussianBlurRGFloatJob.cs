...
namespace Appalachia.Jobs.TextureJobs.Jobs
{
    ...
    public struct GaussianBlurRGFloatJob : IJob
    {
        ...
        private NativeArray<RGFloat> copy;

        private NativeArray<RGFloat> results;
        ...
        public GaussianBlurRGFloatJob(NativeArray<RGFloat> data, int texture_width, int texture_height, float sigma)
        {
            results = data;
            copy = new NativeArray<RGFloat>(data, Allocator.Persistent);
            w = texture_width;
            h = texture_height;
        ...
        private void BoxBlur(NativeArray<RGFloat> src, NativeArray<RGFloat> dst, int r)
        {
            BoxBlurHorizontal(dst, src, r);
            BoxBlurTotal(src, dst, r);
        }

        private void BoxBlurHorizontal(NativeArray<RGFloat> src, NativeArray<RGFloat> dst, int r)
        {
            var iarr = 1f / (r + r + 1);
            for (var i = 0; i < h; i++)
            {
                ...
                for (var j = 0; j <= r; j++)
                {
                    ...
                    dst[ti++] = (RGFloat) math.round(val * iarr);
                }

                for (var j = r + 1; j < (w - r); j++)
                {
                    val += (float2) (src[ri++] - src[li++]);
                    dst[ti++] = (RGFloat) math.round(val * iarr);
                }
        ...
        private void BoxBlurTotal(NativeArray<RGFloat> src, NativeArray<RGFloat> dst, int r)
        {
