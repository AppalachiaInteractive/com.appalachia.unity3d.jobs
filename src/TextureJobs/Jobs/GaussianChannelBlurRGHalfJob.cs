...
{
    [BurstCompile]
    public struct GaussianChannelBlurRGHalfJob : IJob
    {
        ...
        public GaussianChannelBlurRGHalfJob(NativeArray<RGHalf> data, int texture_width, int texture_height, float sigma, int channel)
        {
            results = data;
            copy = new NativeArray<RGHalf>(data, Allocator.TempJob);
            w = texture_width;
            h = texture_height;
        ...
        private void BoxBlur(NativeArray<RGHalf> src, NativeArray<RGHalf> dst, int r, int channel)
        {
            BoxBlurHorizontal(dst, src, r, channel);
            BoxBlurTotal(src, dst, r, channel);
        }

        private void BoxBlurHorizontal(NativeArray<RGHalf> src, NativeArray<RGHalf> dst, int r, int channel)
        {
            var iarr = 1f / (r + r + 1);
            ...
            for (var i = 0; i < h; i++)
            {
                ...
                for (var j = 0; j <= r; j++)
                {
                    ...
                    var newval =(RGHalf) (half2) math.round(val * iarr);
                    var oldval = dst[ti];
        ...
        private void BoxBlurTotal(NativeArray<RGHalf> src, NativeArray<RGHalf> dst, int r, int channel)
        {
        ...
        private static RGHalf UpdateChannel(RGHalf newValue, RGHalf oldValue, int channel)
        {
            var result = oldValue;
