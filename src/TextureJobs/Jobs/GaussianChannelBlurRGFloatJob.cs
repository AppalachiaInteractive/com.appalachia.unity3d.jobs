...
namespace Appalachia.Core.Globals.TextureJobs.Jobs
{
    ...
    public struct GaussianChannelBlurRGFloatJob : IJob
    {
        ...
        public GaussianChannelBlurRGFloatJob(NativeArray<RGFloat> data, int texture_width, int texture_height, float sigma, int channel)
        {
            results = data;
        ...
        private void BoxBlurHorizontal(NativeArray<RGFloat> src, NativeArray<RGFloat> dst, int r, int channel)
        {
            ...
            for (var i = 0; i < h; i++)
            {
                ...
                for (var j = 0; j <= r; j++)
                {
                    ...
                    var newval =(RGFloat) (float2) math.round(val * iarr);
                    var oldval = dst[ti];
