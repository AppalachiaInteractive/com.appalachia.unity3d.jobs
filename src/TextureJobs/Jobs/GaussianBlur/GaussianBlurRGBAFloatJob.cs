...
{
    [BurstCompile]
    public struct GaussianBlurRGBAFloatJob : IJob
    {
        ...
        private void BoxBlur(NativeArray<RGBAFloat> src, NativeArray<RGBAFloat> dst, int r)
        {
            ...
            int r)
        {
            ...
            for (var i = 0; i < h; i++)
            {
                ...
                for (var j = 0; j <= r; j++)
                {
                    ...
                    dst[ti++] = (RGBAFloat) math.round(val * iarr);
                }
                ...
                for (var j = r + 1; j < (w - r); j++)
                {
                    ...
                    val += (float4) src[ri++] - (float4) src[li++];
                    dst[ti++] = (RGBAFloat) math.round(val * iarr);
                }
        ...
        private void BoxBlurTotal(NativeArray<RGBAFloat> src, NativeArray<RGBAFloat> dst, int r)
        {
