...
{
    [BurstCompile]
    public struct GaussianChannelBlurRGB24Job : IJob
    {
        [DeallocateOnJobCompletion]
        private readonly NativeArray<RGB24> copy;

        private readonly NativeArray<RGB24> results;
        ...
        public GaussianChannelBlurRGB24Job(NativeArray<RGB24> data, int texture_width, int texture_height, float sigma, int channel)
        {
            results = data;
        ...
        private void BoxBlur(NativeArray<RGB24> src, NativeArray<RGB24> dst, int r, int channel)
        {
            BoxBlurHorizontal(dst, src, r, channel);
            BoxBlurTotal(src, dst, r, channel);
        }

        private void BoxBlurHorizontal(NativeArray<RGB24> src, NativeArray<RGB24> dst, int r, int channel)
        {
            var iarr = 1f / (r + r + 1);
            ...
            for (var i = 0; i < h; i++)
            {
                ...
                for (var j = 0; j <= r; j++)
                {
                    ...
                    var newval =(RGB24) (int3) math.round(val * iarr);
                    var oldval = dst[ti];
        ...
        private void BoxBlurTotal(NativeArray<RGB24> src, NativeArray<RGB24> dst, int r, int channel)
        {
        ...
        private static RGB24 UpdateChannel(RGB24 newValue, RGB24 oldValue, int channel)
        {
            var result = oldValue;
