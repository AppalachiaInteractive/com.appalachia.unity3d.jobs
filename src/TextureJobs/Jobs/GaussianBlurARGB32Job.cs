...
namespace Appalachia.Core.Globals.TextureJobs.Jobs
{
	...
	public struct GaussianBlurARGB32Job : Unity.Jobs.IJob
	{
		...
		NativeArray<ARGB32> copy;

		NativeArray<ARGB32> results;
		readonly int w, h;
		readonly float sigma;

		[DeallocateOnJobCompletion]
		NativeArray<float> boxes;

		public GaussianBlurARGB32Job(NativeArray<ARGB32> data, int texture_width, int texture_height, float sigma)
		{
			this.results = data;
			this.copy = new NativeArray<ARGB32>(data, Allocator.TempJob);
		...
		void BoxBlur(NativeArray<ARGB32> src, NativeArray<ARGB32> dst, int r)
		{
			BoxBlurHorizontal(dst, src, r);
			BoxBlurTotal(src, dst, r);
		}

		void BoxBlurHorizontal(NativeArray<ARGB32> src, NativeArray<ARGB32> dst, int r)
		{
			...
			for (int i = 0; i < h; i++)
			{
				...
				float4 val = (r + 1) * fv;
				for (var j = 0; j < r; j++) val += (int4) src[ti + j];
				for (var j = 0; j <= r; j++)
				{
					val += (int4) src[ri++] - fv;
		...
		void BoxBlurTotal(NativeArray<ARGB32> src, NativeArray<ARGB32> dst, int r)
		{
			...
			for (int i = 0; i < w; i++)
			{
				...
				float4 val = (r + 1) * fv;
				for (var j = 0; j < r; j++) val += (int4) src[ti + j * w];
				for (var j = 0; j <= r; j++)
				{
					val += (int4) src[ri] - fv;
					dst[ti] = (ARGB32) (int4) math.round(val * iarr);
