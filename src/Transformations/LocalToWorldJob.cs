...
namespace Appalachia.Jobs.Transformations
{
    ...
    {
        [NonSerialized] private static readonly Dictionary<int, TransformLocalToWorld> Data = new Dictionary<int, TransformLocalToWorld>();
        ...
        public static void SetupJob(int guid, NativeArray<float3> positions, ref NativeArray<float3> output)
        {
        ...
        {
            if (!Data.ContainsKey(guid)) return default;
            if (Data[guid].Processing) return default;

            Data[guid].Job = new LocalToWorldConvertJob()
            {
                PositionsWorld = Data[guid].PositionsWorld, PositionsLocal = Data[guid].PositionsLocal, Matrix = localToWorld
            };
